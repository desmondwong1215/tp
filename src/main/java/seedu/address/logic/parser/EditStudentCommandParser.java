...
public class EditStudentCommandParser implements Parser<EditStudentCommand> {
    ...
    public EditStudentCommand parse(String args) throws ParseException {
        ...
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_ID, PREFIX_NAME, PREFIX_PHONE, PREFIX_GENDER);
        ...
        if (argMultimap.getValue(PREFIX_ID).isPresent()) {
            throw new ParseException(EditStudentCommand.MESSAGE_ID_IS_IMMUTABLE);
        }
        if (argMultimap.getValue(PREFIX_NAME).isPresent()) {
            editPersonDescriptor.setName(ParserUtil.parseStudentName(argMultimap.getValue(PREFIX_NAME).get()));
        }
